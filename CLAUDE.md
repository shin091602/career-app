...
動画だけでは「自分が誰で、何を任されていて、いま何を判断しているのか」が伝わらない。
動画を増やさずに補える場所が3つある。

| どこ | フィールド | 中身 |
| --- | --- | --- |
| 最初のタップ画面 | `Episode.prologue` | `{ lines: string[]（3〜4行・各30文字以内）, hint?: 遊び方の一言 }` |
| 選択肢の上 | `Branch.question` | その場面で何を判断しているのか（20文字以内。例：「課長に、どう返す？」） |
| 結果カード | `Episode.lessons` | 「この回で見た仕事のこと」（各40文字以内・4行まで） |

長い説明画面は作らない。**プロローグは読み終えたらタップで本編が始まる**（タイトル画面ではない）。
状況を映像でも見せたいときは、プロローグのクリップを1本足し、
その終点を最初の本編クリップの始点と同じキーフレームにする（`pilot-02` の `p0-desk` が見本）。
...
エピソード側には `prologue`（最初のタップ画面）と `lessons`（結果カード）も書ける。
